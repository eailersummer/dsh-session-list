window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---------------------------------------------------------------------
    var BASE_CSS = [
      ".sl-rail{position:fixed;width:22px;pointer-events:auto;z-index:9;}",
      ...
      ".sl-mark::before{content:\"\";position:absolute;right:0;top:50%;transform:translateY(-50%);width:10px;height:2px;border-radius:1px;background:var(--dsw-alias-label-secondary,rgba(128,128,128,.5));transition:width .14s,background-color .14s;}",
      ".sl-mark:hover::before{background:var(--dsw-alias-brand-primary,#4d6bfe);width:16px;}",
      ".sl-mark.is-active::before{background:var(--dsw-alias-brand-primary,#4d6bfe);width:16px;}",
      ".sl-mark.is-unloaded::before{opacity:.55;width:7px;}",
      ".sl-tip{position:absolute;right:20px;top:50%;transform:translateY(-50%);max-width:300px;max-height:290px;overflow-y:auto;pointer-events:auto;background:var(--dsw-alias-bg-overlay,#fff);border:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.08));border-radius:8px;box-shadow:0 6px 24px rgba(0,0,0,.16);padding:4px 0;}",
