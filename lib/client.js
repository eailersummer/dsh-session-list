window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---------------------------------------------------------------------
    var BASE_CSS = [
      ".sl-rail{position:fixed;width:22px;pointer-events:auto;z-index:9;--sl-accent:#4176e6;--sl-accent-soft:#e4edfd;}",
      "body[data-ds-dark-theme] .sl-rail{--sl-accent:#5686fe;--sl-accent-soft:#34415b;}",
      ...
      ".sl-mark::before{content:\"\";position:absolute;right:0;top:50%;transform:translateY(-50%);width:10px;height:2px;border-radius:1px;background:var(--dsw-alias-label-tertiary,rgba(128,128,128,.45));transition:width .14s,background-color .14s;}",
      ".sl-mark:hover::before{background:var(--sl-accent);width:16px;}",
      ".sl-mark.is-active::before{background:var(--sl-accent);width:18px;}",
      ".sl-mark.is-unloaded::before{opacity:.55;width:7px;}",
      ".sl-tip{position:absolute;right:20px;top:50%;transform:translateY(-50%);max-width:300px;max-height:290px;overflow-y:auto;pointer-events:auto;background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,.08));border-radius:8px;box-shadow:0 6px 24px rgba(0,0,0,.16);padding:4px 0;}",
      ...
      ".sl-tip-item.is-cur{color:var(--sl-accent);background:var(--sl-accent-soft);}",
    ...
    var portalHost = null;
    /**
     * A body-level host for the rail's DOM. The dock entry lives inside
     * `.wSkVaW_composerSeat`, which is `z-index: 7` and therefore a stacking
     * context: anything rendered inside it can never out-paint the width-drag
     * handle (`z-index: 8`). Portalling to `document.body` escapes that
     * context so the rail's own `z-index: 9` actually applies.
     */
    function getPortalHost() {
      if (portalHost === null || !portalHost.isConnected) {
        portalHost = document.createElement("div");
        portalHost.setAttribute("data-plugin", "dsh-session-list-rail");
    ...
    function QuestionRail(props) {
      ...
      var railElement = React.createElement("div", {
        className: "sl-rail",
      ...
      if (ReactDOM && typeof ReactDOM.createPortal === "function") {
        return ReactDOM.createPortal(railElement, getPortalHost());
      }
      return railElement;
    }
    ...
    function apply(ctx) {
      ctx.effect(function () {
        ...
        return function () {
          base.remove();
          if (portalHost !== null) { portalHost.remove(); portalHost = null; }
        };
