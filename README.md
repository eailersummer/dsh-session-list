...
- **配色跟随主题**：全部使用 DSH 主题变量（`--dsw-alias-*`），亮/暗色主题自动适配；
...
- **`--dsw-alias-brand-primary` 在 0.1.5 是中性强对比色**（亮色主题 `#0f1115` / 暗色主题 `#f9fafb`），并非蓝色 accent，所以 hover / 当前态横线是黑白高亮；想要蓝色可改用 `--dsw-alias-state-business-primary`。
